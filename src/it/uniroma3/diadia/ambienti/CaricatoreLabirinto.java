...
public class CaricatoreLabirinto {
	...
	private void leggiECreaStanzeBloccate() throws FormatoFileNonValidoException {
		...
		for (String specificaStanzaBloccata : separaStringheAlleVirgole(specificheStanzeBloccate)) {
			...
			Direzione dir = null;
			try {
				dir = Direzione.valueOf(nomeDirezioneBloccata.toUpperCase());
			}catch(IllegalArgumentException e) {
				check(false, "Direzioe non valida "+ nomeDirezioneBloccata);
			}
			
			this.builder.addStanzaBloccata(nomeStanzaBloccata, dir, nomeAttrezzoSblocco);
		}
	}
	...
	private void leggiECollocaAttrezzi() throws FormatoFileNonValidoException {
		...
		for (String specificaAttrezzo : separaStringheAlleVirgole(specificheAttrezzi)) {
			...
			posaAttrezzo(nomeAttrezzo, pesoAttrezzo);
		}
	}

	private void posaAttrezzo(String nomeAttrezzo, String pesoAttrezzo)
	...
	private void aggiungiMago(String nomePersonaggio, String presentazionePersonaggio, String nomeAttrezzo,
		...
		try {
			...
			this.builder.addMago(nomePersonaggio, presentazionePersonaggio, stanza);
		} catch (NumberFormatException e) {
			check(false, "Peso attrezzo " + pesoAttrezzo + " non valido");
		}
	}
	...
	private void leggiECollocaCani() throws FormatoFileNonValidoException {
		...
		for (String specificoCane : separaStringheAlleVirgole(specificheCani)) {
			...
			this.builder.addCane(nomeCane, presentazioneCane, nomeStanza);
		}
	}
	...
	private void leggiEImpostaUscite() throws FormatoFileNonValidoException {
		...
		for (String specificaUscita : separaStringheAlleVirgole(specificheUscite)) {
			...
			try (Scanner scannerDiLinea = new Scanner(specificaUscita)) {
				...
				nomeDirezione = scannerDiLinea.next();
				check(scannerDiLinea.hasNext(), msgTerminazionePrecoce(
						"la destinazione di una uscita della stanza " + stanzaPartenza + " nella direzione " + nomeDirezione));
				stanzaDestinazione = scannerDiLinea.next();
			}
			Direzione dir = null;
			try {
				dir = Direzione.valueOf(nomeDirezione.toUpperCase());
			}catch(IllegalArgumentException e) {
				check(false, "Direzioe non valida "+ nomeDirezione);
			}
			
			impostaUscita(stanzaPartenza, stanzaDestinazione, dir);
		}
	}
	...
	private void impostaUscita(String stanzaPartenza, String stanzaArrivo,  Direzione dir) throws FormatoFileNonValidoException {
		check(isStanzaValida(stanzaPartenza), "Stanza di partenza sconosciuta " + dir);
		check(isStanzaValida(stanzaArrivo), "Stanza di destinazione sconosciuta " + dir);
		this.builder.addAdiacenza(stanzaPartenza, stanzaArrivo, dir);
	}
